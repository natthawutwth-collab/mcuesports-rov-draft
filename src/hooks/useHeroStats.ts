...
export function useHeroStats(selectedHeroName: string | null = null, oppPicks: string[] = []) {
  ...
  // Played With (เล่นกับ) - Synergies
  const playedWith: HeroSynergy[] = useMemo(() => {
    if (!selectedHeroName) return [];
    return statsDataProvider.getHeroPlayedWith(selectedHeroName);
  }, [selectedHeroName, status]);

  // Played Against (เจอกับ) - Matchups
  const playedAgainst: HeroMatchup[] = useMemo(() => {
    if (!selectedHeroName) return [];
    return statsDataProvider.getHeroPlayedAgainst(selectedHeroName);
  }, [selectedHeroName, status]);
